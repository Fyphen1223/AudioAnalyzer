...
export function drawWaveformAndMeter({ state, dom, frame }) {
  ...
  if (truePeakDb >= 0.0) {
    ...
    if (!state.lastClipTime || now - state.lastClipTime > 500) {
      ...
      if (dom.clipLogContainer) {
        dom.clipLogContainer.innerHTML = state.clipLogs.map((log) => `<div style="color: #ef4444; border-bottom: 1px solid var(--border); padding: 2px 0;">${log}</div>`).join("");
      }
    }
  }
  ...
  if (dom.peakFill.style.width !== widthStr) dom.peakFill.style.width = widthStr;

  let textContent = "-\u221E dB";
  if (state.prevPeakValue > -100) {
    textContent = state.prevPeakValue.toFixed(1) + " dB" + (meterOffset ? ` (${standard})` : "");
  }

  if (dom.peakValue.textContent !== textContent) dom.peakValue.textContent = textContent;
  if (dom.peakFill.style.backgroundColor !== "#e2e8f0") dom.peakFill.style.backgroundColor = "#e2e8f0";
  if (dom.peakValue.style.color !== "#e2e8f0") dom.peakValue.style.color = "#e2e8f0";
}
