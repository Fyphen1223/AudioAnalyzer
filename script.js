...
const meteringStandard = document.getElementById("metering-standard");
...
function resizeCanvases() {
  ...
  if (canvasSpectrum) {
    ...
    ctxSpectrum.setTransform(1, 0, 0, 1, 0, 0);
  ...
  if (canvasWaveform) {
    ...
    ctxWaveform.setTransform(1, 0, 0, 1, 0, 0);
  ...
  if (canvasSpectrogram) {
    ...
    canvasSpectrogram.style.width = specGramRect.width + "px";
    canvasSpectrogram.style.height = specGramRect.height + "px";
  }
  ...
  if (canvasVectorscope) {
    ...
    ctxVectorscope.scale(dpr, dpr);
    canvasVectorscope.style.width = vectorscopeRect.width + "px";
    canvasVectorscope.style.height = vectorscopeRect.height + "px";
  }
}
...
async function startAudio() {
  try {
    ...
    if (channelCount > 1) {
      ...
      // Connect mono to right as well to avoid silent right channel
...
function draw(timestamp) {
  ...
  const standard = meteringStandard ? meteringStandard.value : "peak";
  ...
  if (maxAbs > 0) {
    if (standard === "rms") {
      ...
      // Very naive LUFS approximation (RMS + 3dB)
      const rms = Math.sqrt(sumSquares / analyser.fftSize);
  ...
  // Determine standard offset for visualization meter ranges
  let meterOffset = 0;
  if (standard === "k-12") {
    meterOffset = -12;
    currentPeakDb = 20 * Math.log10(maxAbs);
    ...
    meterOffset = -20;
    currentPeakDb = 20 * Math.log10(maxAbs);
  } else if (standard.startsWith("k-")) {
    currentPeakDb = 20 * Math.log10(maxAbs);
  }

  let displayDb = prevPeakValue - meterOffset;
  if (displayDb < -60) displayDb = -60;

  // Convert -60..0 db to 0..100%
  const meterPercent = Math.max(0, (displayDb + 60) / 60) * 100;

  peakFill.style.width = Math.min(100, meterPercent) + "%";
  ...
  // --- 5. Draw Vectorscope & Correlation ---
  if (ctxVectorscope && canvasVectorscope && analyserL && analyserR) {
    ...
    if (correlationValue && correlationFill) {
      let corr = 0;
      if (normL > 0 && normR > 0) {
        corr = dot / Math.sqrt(normL * normR);
      } else if (normL > 0 || normR > 0) {
        corr = 0; // one channel quiet
      } else {
